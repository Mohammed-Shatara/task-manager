...
class TaskValidationUseCase
    extends UseCase<Result<BaseError, bool>, TaskParams> {
  ...
  Result<BaseError, bool> call(TaskParams params) {
    ...
    final statusError = BaseValidator.validateValue(params.status, [
      requiredValidator,
    ], true);

    final isDueDateValid = dateValidator.validateFunction(params.dueDate);
    final dueDateError =
        isDueDateValid ? null : dateValidator.getValidateMessage();
...
class UpdateTaskValidationUseCase
  ...
  Result<BaseError, bool> call(UpdateTaskParams params) {
    ...
    if (idError != null) {
      return Result(error: CustomError(message: idError));
    }
